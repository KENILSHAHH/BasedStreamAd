...
const Onboarding = () => {
  ...
  const [isLoadingPage, setIsLoadingPage] = useState(loading);
  const router = useRouter();

  const handleLogin = async () => {
    const walletAddress = address; // Replace with actual wallet address logic
    try {
    ...
    const fetchTwitterStatus = async () => {
      try {
        const response = await fetch(
          `https://streamads-python-backend.onrender.com/twitter_status?wallet_address=${address}`,
          {
            method: "GET",
            redirect: "follow",
          }
  ...
  }, [isConnected]); // Add checkedWallet as a dependency
  // useEffect(() => {
