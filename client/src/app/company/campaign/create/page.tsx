...
const Create = () => {
  ...
  const handleFinish = async () => {
    setLoading(true);
    try {
       const aptAmount = prizePool?.toString();
	  if (!aptAmount) {
		throw new Error("Prize pool amount is not defined");
	  }
      const WLD = ethers.utils.parseUnits(aptAmount, 18);
      
      const contractAddress = "0xE7E1C768b46d6e7b2b32810921475ee88b5CB475"
      const contractAbi = [
	{
		"inputs": [],
	...
	},
	{
      ...
      const tokenAddress = "0x0B0F64952a3041766e9769F87013b906884BA9A2"
      const tokenAbi = [
	{
		"inputs": [
			{
	...
	},
	{
     ...
     const web3Modal = new Web3Modal();
      ...
      const approve = await tokenContract.approve(contractAddress, WLD);
      console.log(approve);
      await approve.wait(2);
      const response = await contract.createCampaign(tokenAddress, WLD);
